<?php
	...
	$light = array('#F0F0F0', '#444', '#111', '#444', '#444', '#666', '#333', '#999', '#AAA', '#DDD', '#555', '#111'   , '#222', '#444', '#555', '#444', '#999', '#777', 'top' => '#707070');
	$dark  = array('#0F1A0F', '#888', '#AAA', '#AAA', '#888', '#777', '#999', '#444', '#555', '#AAA', '#222', '#111'   , '#666', '#888', '#777', '#999', '#555', '#666', 'top' => '#AAA');
	$black = array('#000000', '#999', '#CCC', '#AAA', '#888', '#777', '#999', '#111', '#222', '#222', '#AAA', '#0D0D0D', '#333', '#666', '#888', '#AAA', '#444', '#666', 'top' => '#AAA');
...
body 					{ color: #BBB; background: <?php echo $s[0]; ?>; font-size: 11px; margin: 0px; padding: 0px; font-family: Tahoma, Arial, Helvetica, Serif; direction: ltr; }
...
div.topPhorm a			{ color: <?php echo $s['top']; ?>; }
...
div.footer a			{ color: <?php echo $s[5]; ?>; }
...
.part .titlepart	 	{ color: <?php echo $s[3]; ?>; position: relative; left: -10px; font-size: 8pt; margin: 0px 25px 8px 0px; padding: 5px 5px 5px 10px; background: url('files/cssfiles/<?php echo $c; ?>_hbg1.jpg') right repeat-y; }
...
.categeach				{ }
.partmain				{ line-height: 100%; background: <?php echo $s[0]; ?>; margin: 0px 15px 0px 0px; text-align: left; }
.partmain .submenu		{ clear: both; border: 1px solid <?php echo $s[0]; ?>; }
.partmain .titlepart	{ color: <?php echo $s[1]; ?>; letter-spacing: 5px; margin: 0px 20px 10px 0px; padding: 15px 15px 10px;  background: url('files/cssfiles/<?php echo $c; ?>_vbg1.jpg') bottom; }
...
.partmain .end			{ text-align: left; clear: both; margin: 0px 20px 30px 0px; background: url('files/cssfiles/<?php echo $c; ?>_vbg0.jpg') top; padding: 7px 20px 30px 20px; }
.titlepartlinkR			{ color: #666; float: right; letter-spacing: 0px; }
.titlepartlinkR a		{ color: <?php echo $s[5]; ?>; }
.titlepartlinkR a:hover	{ color: <?php echo $s[3]; ?>; }
...
.photoTheImg img		{ margin: 5px 15px 30px 0px; border: 1px solid #000000; }
...
.alert_msg				{ color: #643; letter-spacing: 1px; text-align: center; background: #D8D4D0; width: 80%; margin: 5px auto 15px; padding: 10px 15px; border: 3px double #999; }
.ok_msg					{ color: <?php echo $s[1]; ?>; letter-spacing: 1px; text-align: center; background: #C0C8C4; width: 80%; margin: 5px auto 15px; padding: 10px 15px; border: 3px double #999; }
...
.Commenting .cell		{ background: url('files/cssfiles/<?php echo $c; ?>_vbg0.jpg') top; padding: 10px 20px 5px; margin: 10px 20px 20px; }
