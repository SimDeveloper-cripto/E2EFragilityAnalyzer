	...
	if (strcmp($feat, "slideshow") == 0) {
		...
		if (count($r) != 0 ) {
			...
			while (list($key, $val) = each($r)) {
				...
				$dates = sscanf($r[$key]['dateadd'], "%d/%d/%d %d:%d");
				$r[$key]['date'] = date("M jS y, h:i", mktime($dates[3], $dates[4], 0, $dates[1], $dates[2], $dates[0]));
			}
			reset($r);
...
?>
	<div class="partmain">
		<div class="titlepart" style="letter-spacing: 2px;">
			<span class="leaveReply" id="ss_date"><?php echo $r0['date']; ?></span>
			<span class="reddot" style="padding: 0px 10px;">&#149;</span>
			SlideShow of <?php echo $stitle; ?> ::
			<a id="ss_link1" href="?p=<?php echo key($r); ?>" style="letter-spacing: 5px;">
				<span id="ss_title"><?php echo $r0['name']; ?></span>
			</a>
		</div>
			...
			<div style="margin: 0px auto 5px; width: 90%">
				<table style="width: 100%"><tr>
					<td style="text-align: left" width="25%">
						...
						[ <a class="q" id="ss_playpause_link" onclick="javascript:ss_playpause();">Pause</a> ]
						&nbsp; &nbsp;
		...
		<div style="text-align: center; margin: 15px 0px 10px;">
			<a id="ss_link2" href="?p=<?php echo key($r); ?>" style="display:block">
				<img id="ss_photo" src="<?php echo $r0['src']; ?>" />
			</a>
			<div id="ss_theend" style="display: none;" class="">
				<b>This is the end of SlideShow <?php if (strlen($theend)) echo " of last ".($ns+$threshold)." photos"; ?>.</b><br /><br />
