<?php
	...
	define("DEFAULT_JB_N", 50);
	$basis['defrpc'] = 20; // recent photos in index
	$basis['deftrc'] = 10; // top rated in index
	$basis['defrsc'] = 5;  // stories in index
	$basis['defrss'] = 10; // stories in side bar

	if (strcmp($basis['theme'], 'random') == 0) {
		$handle = opendir('./files');
		$themes = array();
		while (false !== ($file = readdir($handle)))
			if ($file != "." && $file != ".." && (strcasecmp(substr($file, -4), ".css") == 0))
				array_push($themes, $file);
		$basis['theme'] = $themes[array_rand($themes)];
	}
	if (strcmp($basis['mode'], 'random') == 0) {
		$modes = array('photos', 'stories', 'box');
		$basis['mode'] = $modes[array_rand($modes)];
	}
	...
	if (($p != -1) && !isset($photos[$p]) && ($p != $photos['lastpid']))
		$alert_query = "The requested photo [pid = $p] does not exist!";
	if (($c != -1) && !isset($categs[$c]) && ($c != $categs['lastcid']))
		$alert_query = "The requested category [cid = $c] does not exist!";
	if (($s != -1) && !isset($stories[$s]) && ($s != $stories['lastsid']))
	...
	if (strlen($cmd) && !$bad_query)
		switch ($cmd) {
			...
			case 'login':
				if (!isset($_POST['pass']))
					$alert_msg = "Password field can not be left blank.";
				else {
					$carr = array();
					$let = ($c != -1)?"c":"s";
					$carr = ($c != -1)?$categs[$c]:$stories[$s];
			...
			case 'addcmntp':
				if (!isset($_POST['name']))
					...
					if (strcmp($photo['getcmnts'], 'yes') != 0)
						...
						if (isset($basis['wvw']))
							$alert_msg .= " You should have entered \"".$basis['wvw']."\", but you did \"".$_POST['wvw']."\"";
						}
					else {
	...
	if ($p != -1) {
		...
		if (!checkThePass("s", $photo['story'])) {
			...
			if (strlen($cmd) == 0) {
				...
				$hits = $rating[0];
				$hits++;
				$photos[$p] = sprintf("%d %s", $hits, $rating[1]);
				save_container('photos', 'Photo', './data/photos.xml');
			}
			...
			echo "\t<div class=\"pvTitle\">"
					."<span style=\"float: right; margin-top: 15px; \">"
						."<a target=\"_top\" href=\"javascript:toggleInfo();\" style=\"font-size: 11px; font-weight: normal; letter-spacing: 1px; \">"
							." [ <span id=\"hin\">Hide&nbsp;</span> info ]"
						."</a>"
						."<a style=\"margin: 0px 10px; font-size: 11px; font-weight: normal; letter-spacing: 1px; \" "
						."href=\".?c={$photo['categ']}\">[ Up ]</a>"
					."</span>";
			if (strlen($name))
				...
				echo "&nbsp;";
			echo "</div>\n";
			$photoDesc = (strlen($photo['desc']) > 0)?nl2br(noquo($photo['desc'])):"No Descripton.";
			$photoInfo = nl2br(noquo($photo['photoinfo']));
			...
			echo "\t\t<div class=\"photoBox\">\n";
				...
				echo $spc;
				echo "\t\t\t$dd Photo ID: <a href=\".?p=$p\">$p</a><br />\n";
...
<?php
	$tr = array_flip(get_html_translation_table(HTML_SPECIALCHARS, ENT_NOQUOTES));
	$basis['extra'] = strtr($basis['extra'], $tr);
